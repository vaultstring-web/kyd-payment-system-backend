...
BEGIN
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND NEW.currency <> OLD.currency) THEN
    ...
    ELSIF cc = 'MW' AND NEW.currency <> 'MWK' THEN
      RAISE EXCEPTION 'currency not allowed for user country';
    ELSIF cc = 'ZM' AND NEW.currency <> 'ZMW' THEN
      RAISE EXCEPTION 'currency not allowed for user country';
    ELSIF cc <> 'CN' AND cc <> 'MW' AND cc <> 'ZM' AND NEW.currency <> 'ZMW' AND NEW.currency <> 'MWK' THEN
      RAISE EXCEPTION 'currency not allowed for user country (must be ZMW or MWK for others)';
    END IF;
  END IF;
